...
export function defineMemorizeTool(memory: MemoryService) {
  return defineTool({
    name: 'memory_memorize',
    description:
      'Write a fact or preference into the long-term memory store. Idempotent: '
    ...
    async execute(args: MemorizeArgs, exec: { agent?: { id?: string } }) {
