...
import { STATE_LAST_IMPORT_SEQ } from '../provider/sqlite/local-provider.ts'
import { DSH_WRITTEN_MARK } from './paths.ts'
import { parseJsonl, splitSentinel } from './renderer.ts'
 ...
 */
export async function importJsonl(provider: SQLiteLocalProvider, jsonlText: string): Promise<ImportResult> {
  const rows = parseJsonl(jsonlText)
  const seq = Number(provider.getState(STATE_LAST_IMPORT_SEQ) ?? '0')
  if (rows.length <= seq) return { imported: 0, archived: 0, migrated: 0 }
  ...
  for (const row of fresh) {
    if (row.action === 'upsert' && row.content) {
      provider.upsert({
        content: row.content,
        importance: (row.importance ?? 1) as 1 | 2 | 3,
        scope: (row.scope ?? 'session') as 'session' | 'user' | 'global',
 ...
 */
export async function migrateSentinelOnce(
