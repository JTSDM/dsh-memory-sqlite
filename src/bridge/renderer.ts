...
import { STATE_LAST_RENDER_SEQ } from '../provider/sqlite/local-provider.ts'
import { DSH_MEMORY_END, DSH_MEMORY_START, DSH_WRITTEN_MARK } from './paths.ts'
import type { JsonlRow } from './exporter.ts'

/** Render one JSONL row as a Markdown entry (upsert or archive marker). */
export function renderRowToMarkdown(row: JsonlRow): string {
 ...
 */
export function splitSentinel(text: string): { before: string; inside: string | null; after: string } {
...
export function ensureSentinel(text: string): string {
  const split = splitSentinel(text)
  if (split.inside !== null) return text
  const trimmed = split.before.replace(/\s+$/, '')
  return `${trimmed}\n\n${DSH_MEMORY_START}\n${DSH_MEMORY_END}\n`
}

/** Append rendered rows before the END marker (incremental, never a redraw). */
export function appendToSentinel(text: string, entries: string[]): string {
  const split = splitSentinel(text)
