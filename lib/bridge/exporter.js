...
import { STATE_LAST_EXPORT_SEQ } from "../provider/sqlite/local-provider.js";
...
export async function jsonlLineCount(path) {
    try {
 ...
 */
export async function exportCoreMemories(provider, jsonlPath, config = DEFAULT_EXPORT_CONFIG) {
    const now = Date.now();
    ...
    const core = provider.listByStatus('active').filter(m => m.importance >= config.exportMinImportance
 ...
 */
export async function appendArchiveRow(provider, jsonlPath, memory) {
    const exported = provider.bridgeGetExported(memory.id);
    if (exported !== 'upsert')
        return false;
    await appendJsonlRow(jsonlPath, {
...
export function dshWrittenMark() {
