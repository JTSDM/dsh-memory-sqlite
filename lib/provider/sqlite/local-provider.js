...
import { contentHash, normalizeContent } from "./normalize.js";
...
const SCOPE_PRIORITY = { session: 0, user: 1, global: 2 };
...
export class SQLiteLocalProvider {
    ...
    db;
    reinforceWindowMs;
    constructor(dbPath, reinforceWindowMs = DEFAULT_REINFORCE_WINDOW_MS) {
        ...
        for (const key of STATE_KEYS) {
            if (this.getState(key) === null)
                this.setState(key, key === STATE_REINFORCE_DELTA ? '0' : '');
        }
    }
    open(path) {
    ...
    upsert(candidate) {
        ...
        if (!content)
            throw new Error('memory content must not be empty after normalization');
        const hash = contentHash(content);
        const now = Date.now();
        const existing = this.db.prepare('SELECT * FROM memories WHERE content_hash = ?').get(hash);
        if (!existing) {
            const id = randomUUID();
            ...
            this.db.prepare(`INSERT INTO memories (id, content, content_hash, importance, type, scope, session_id, status,
           reinforce_count, ttl_ms, last_confirmed_at, source_ref, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'active', 0, ?, ?, ?, ?, ?)`).run(id, content, hash, importance, type, scope, sessionId, DEFAULT_TTL_MS, now, candidate.sourceRef ?? null, now, now);
            return {
        ...
        const importance = clampImportance(candidate.importance ?? existing.importance);
        ...
        const nextImportance = Math.max(Number(existing.importance), importance);
        const nextType = (candidate.type ?? existing.type);
        const nextScope = (candidate.scope ?? existing.scope);
        const nextSourceRef = candidate.sourceRef ?? existing.source_ref;
        ...
        const nextSessionId = nextScope === 'session'
            ? (candidate.sessionId ?? existing.session_id)
            : null;
        if (status === 'archived') {
            // Row 5: archived → restore, reinforceCount = old + 1.
            this.db.prepare(`UPDATE memories SET status = 'active', reinforce_count = reinforce_count + 1,
           importance = ?, type = ?, scope = ?, session_id = ?, last_confirmed_at = ?, source_ref = ?, updated_at = ?
         WHERE id = ?`).run(nextImportance, nextType, nextScope, nextSessionId, now, nextSourceRef, now, String(existing.id));
            this.bumpDelta();
            return {
        ...
        this.db.prepare(`UPDATE memories SET importance = ?, type = ?, scope = ?, session_id = ?,
         last_confirmed_at = ?, source_ref = ?, updated_at = ? WHERE id = ?`).run(nextImportance, nextType, nextScope, nextSessionId, now, nextSourceRef, now, String(existing.id));
        return {
    ...
    bumpDelta() {
        this.db.prepare(`INSERT INTO engine_state (key, value) VALUES (?, '1')
       ON CONFLICT(key) DO UPDATE SET value = CAST(CAST(value AS INTEGER) + 1 AS TEXT)`).run(STATE_REINFORCE_DELTA);
    }
    recall(query) {
        const scopes = query.scopes ?? ['session', 'user', 'global'];
        ...
        if (query.text) {
            ...
            let fts = [];
            try {
                fts = this.db.prepare(`SELECT f.rowid FROM memories_fts f WHERE memories_fts MATCH ? ORDER BY rank LIMIT ?`).all(query.text, Math.max(limit * 5, 50));
            }
            catch {
                fts = [];
            }
            if (fts.length > 0) {
                ...
                const rows = this.db.prepare(`SELECT * FROM memories WHERE rowid IN (${[...ids].join(',')})
             AND ${scopeAndIsolation} AND ${statusClause}
             AND (? IS NULL OR last_confirmed_at >= ?)
           ORDER BY importance DESC, last_confirmed_at DESC`).all(...isoArgs, query.sinceMs ?? null, query.sinceMs ?? null);
         ...
         WHERE content LIKE ? ESCAPE '\\' AND ${scopeAndIsolation} AND ${statusClause}
           AND (? IS NULL OR last_confirmed_at >= ?)
         ORDER BY importance DESC, last_confirmed_at DESC LIMIT ?`).all(like, ...isoArgs, query.sinceMs ?? null, query.sinceMs ?? null, Math.max(limit * 5, 50));
            for (const row of rows) {
                const memory = toMemory(row);
            ...
            const rows = this.db.prepare(`SELECT * FROM memories WHERE ${scopeAndIsolation} AND ${statusClause}
           AND (? IS NULL OR last_confirmed_at >= ?)
         ORDER BY importance DESC, last_confirmed_at DESC LIMIT ?`).all(...isoArgs, query.sinceMs ?? null, query.sinceMs ?? null, limit);
            core = rows.map(row => ({
                memory: toMemory(row),
                score: scopeWeight(row.scope) + Number(row.importance) * 10,
            }));
        }
        ...
        if (anchor && core.length > 0) {
         ...
         ORDER BY importance DESC, last_confirmed_at DESC LIMIT ?`).all(anchor.scope, anchor.type, query.sessionId ?? '', ...coreIds, limit);
            related = rows.map(row => ({
                memory: toMemory(row),
                score: scopeWeight(row.scope) + Number(row.importance) * 5,
            }));
        }
        ...
        if (query.includeArchive || query.text) {
            const archived = this.db.prepare(`SELECT * FROM memories WHERE status = 'archived' AND ${scopeAndIsolation}
         ORDER BY updated_at DESC LIMIT ?`).all(...isoArgs, limit);
    ...
    decay() {
        const now = Date.now();
        const result = this.db.prepare(`UPDATE memories SET status = 'dormant', updated_at = ?
       WHERE status = 'active' AND ttl_ms IS NOT NULL AND last_confirmed_at < ? - ttl_ms`).run(now, now);
    ...
    archive(id, reason) {
        ...
        if (!existing || existing.status === 'archived')
            return null;
        const now = Date.now();
        this.db.prepare(`UPDATE memories SET status = 'archived', updated_at = ? WHERE id = ?`).run(now, id);
        this.db.prepare(`INSERT INTO archive (id, memory_id, archived_at, reason, snapshot) VALUES (?, ?, ?, ?, ?)`).run(randomUUID(), id, now, reason ?? null, JSON.stringify(existing));
        return this.get(id);
    }
    restore(id) {
        const existing = this.get(id);
        if (!existing || existing.status !== 'archived')
            return null;
        const now = Date.now();
        this.db.prepare(`UPDATE memories SET status = 'active', last_confirmed_at = ?, updated_at = ? WHERE id = ?`).run(now, now, id);
        this.bumpDelta();
        return this.get(id);
    }
    get(id) {
    ...
    findByHash(contentHashValue) {
        const row = this.db.prepare('SELECT * FROM memories WHERE content_hash = ?').get(contentHashValue);
        return row ? toMemory(row) : null;
    }
    listByStatus(status) {
        const rows = this.db.prepare('SELECT * FROM memories WHERE status = ?').all(status);
    ...
    setState(key, value) {
        this.db.prepare(`INSERT INTO engine_state (key, value) VALUES (?, ?)
    ...
    // ---------------------------------------------------------------------------
    /** Durable queue row. */
    outboxEnqueue(sessionId, payload) {
        const id = randomUUID();
        const now = Date.now();
        this.db.prepare(`INSERT INTO outbox (id, session_id, payload, status, created_at, updated_at, attempts, last_error)
       VALUES (?, ?, ?, 'pending', ?, ?, 0, NULL)`).run(id, sessionId, payload, now, now);
        return id;
    }
    /**
     ...
     */
    outboxClaim(maxAttempts) {
        const now = Date.now();
        const stale = now - 5 * 60 * 1000;
        this.db.prepare(`UPDATE outbox SET status = 'pending'
       WHERE status = 'processing' AND updated_at < ?`).run(stale);
        const row = this.db.prepare(`SELECT * FROM outbox
       WHERE status = 'pending' OR (status = 'failed' AND attempts < ?)
       ORDER BY created_at ASC LIMIT 1`).get(maxAttempts);
        if (!row)
            return null;
        this.db.prepare(`UPDATE outbox SET status = 'processing', updated_at = ? WHERE id = ?`).run(now, String(row.id));
        return { ...toOutboxRow(row), status: 'processing' };
    }
    outboxComplete(id) {
        this.db.prepare(`UPDATE outbox SET status = 'done', updated_at = ? WHERE id = ?`).run(Date.now(), id);
    }
    outboxFail(id, error, maxAttempts) {
        const attempts = this.db.prepare('SELECT attempts FROM outbox WHERE id = ?').get(id);
        const next = (attempts?.attempts ?? 0) + 1;
        this.db.prepare(`UPDATE outbox SET status = ?, attempts = ?, last_error = ?, updated_at = ? WHERE id = ?`).run(next >= maxAttempts ? 'failed' : 'pending', next, String(error), Date.now(), id);
    }
    outboxCount(status) {
        const row = this.db.prepare('SELECT COUNT(*) AS n FROM outbox WHERE status = ?').get(status);
        return Number(row.n);
    }
    /** Whether a session already has an unfinished outbox row (dedup enqueue). */
    outboxHasActive(sessionId) {
        const row = this.db.prepare(`SELECT 1 AS x FROM outbox WHERE session_id = ? AND status IN ('pending','processing') LIMIT 1`).get(sessionId);
    ...
    bridgeMarkExported(id, lastAction) {
        this.db.prepare(`INSERT INTO bridge_exported (id, last_action, exported_at) VALUES (?, ?, ?)
       ON CONFLICT(id) DO UPDATE SET last_action = excluded.last_action, exported_at = excluded.exported_at`).run(id, lastAction, Date.now());
    }
    /** Last JSONL action for a memory, or null when never exported. */
    bridgeGetExported(id) {
        const row = this.db.prepare('SELECT last_action FROM bridge_exported WHERE id = ?').get(id);
...
function capByTokens(items, maxTokens) {
    let used = 0;
    let kept = 0;
    for (const item of items) {
        const approx = Math.ceil(item.memory.content.length / 4);
        if (used + approx > maxTokens)
            break;
        used += approx;
        kept += 1;
    }
